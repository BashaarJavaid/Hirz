...
**Phase 3 item 17 is partial.** Read-only planning, three scenario checks and offline reproduction pass; historical hard-constraint failures leave 2/365 EV-only days and no solar/battery comparison days. Item 17 remains next; execution and persistence stay deferred. Item 15's physical plug/absence checks remain pending. Evidence: `docs/verification-log.md`; procedures: `docs/development.md`. Development remains on `0005_execution_attempt`; upgrades are manual. No new remote CI run.
