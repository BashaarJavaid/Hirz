...
- **Approved item 22 disposable-scenario bootstrap (2026-09-22):** the isolated scenario may install its seed, explicitly supplied room metadata, bindings, rate plan and EV needed-by time before execution. Subsequent persisted changes and all device writes require Pipeline decisions. Simulated v8 preview never activates the seeded v7 execution policy or fabricates activation audit events (ADR-006).
...
- `uv run python scripts/smoke_explainer.py` — offline narration and invented-figure guard; `--integration --audit-output <new-file>` checks audited publication and restart reuse in a disposable database. No development migration or live Bedrock call. Procedures: `docs/development.md`.
- `uv run hirz scenario run scenarios/demo-evening.yaml --headless --assert` — execute the evening in a disposable PostgreSQL database; `--speed 60` instead paces simulated time. `--artifacts-dir <new-dir>` retains private report/audit/public-key evidence, and `--output <new-file>` writes a separate report. `hirz scenario step <file> --to "18:40"` (or `run --step`) exits before target-time events; later checks are not reached. `uv run python scripts/smoke_scenario.py --live-demo --artifacts-dir <new-dir>` verifies and restores the real HA demo lamp at normal speed. Procedures: `docs/development.md`.
...
**Phase 4 item 24 is locally verified; item 25 (household tools) is next.**
Local simulated OAuth, scopes and member mapping passed SDK/browser checks;
anonymous onboarding remains available. Evidence: `docs/verification-log.md`;
procedures: `docs/development.md`. Development remains on `0005_execution_attempt`;
migrations 0006–0009 are manual. Physical-plug checks, household tool isolation,
real/companion login, trust/security execution, Link, authenticated activation,
live Bedrock and remote CI remain unverified.
