...
function CardApp() {
  ...
  return <main className={full ? "canvas fullscreen" : "canvas"} aria-label="Hirz household card" aria-busy={busy}>
    ...
    <footer>
      ...
      {card?.kind === "doorbell" && card.can_request && card.room && <button className="primary" disabled={disabled} onClick={() => void call({ name: "execute_household_action", arguments: { action: "request_door_unlock", minutes: 10, room: card.room! } }, true)}>Request 10-minute unlock</button>}
