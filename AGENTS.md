...
**Phase 2 item 15 software is verified; its physical plug gate is next.** Local HA reads, subscriptions, single-attempt Pipeline writes and scenario-only read fallback are built. The physical plug write/state/power and absence checks remain pending. Evidence: `docs/verification-log.md`; procedures: `docs/development.md`. Development remains on `0003_pipeline`; upgrades are manual. General execution, ingestion and scenarios remain later work. Four CI jobs remain placeholders; no new remote CI run.
